<script lang="ts">
	...
	onMount(async () => {
		...
		const [colSnap, nomSnap] = await Promise.all([getDocs(colleaguesRef), getDocs(nominationsRef)]);
	...
	<section class="leaderboard-page" in:fade={{ duration: 300 }}>
		...
		{:else}
			...
			<section class="top-three">
				{#if rows[0]}
					<article
						class="card top-card top-gold"
						in:fly={{ y: 12, duration: 200, delay: 40 }}
					>
				...
				{#if rows[1]}
					<article
						class="card top-card top-silver"
						in:fly={{ y: 12, duration: 200, delay: 80 }}
					>
				...
				{#if rows[2]}
					<article
						class="card top-card top-bronze"
						in:fly={{ y: 12, duration: 200, delay: 120 }}
					>
				...
				<section class="card rest-card">
					...
					<ul class="rest-list">
						{#each rows.slice(3) as row, i}
							<li in:fade={{ duration: 120, delay: 50 + i * 25 }}>
								<div class="left">
